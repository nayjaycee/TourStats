...
/* ====== Floating image (if you still use it) ====== */
.dd-float-wrap{
  width: 100%;
  display:flex;
  justify-content:center;
...
def compute_ytd_from_fields_today(fields: pd.DataFrame, year: int = 2026) -> pd.DataFrame:
    """
    YTD is computed ONLY from rows that:
      1) are in the given year, AND
      2) have an actual recorded 0/1 in at least one outcome column
         (blank/null means event not completed -> do not count)

    Starts = number of completed events (unique by event_id if present)
    Made cut % = made_cuts / starts
    """
    ...
    if "year" not in df.columns:
        raise ValueError("Fields.xlsx must contain a 'year' column for YTD logic.")
    df["year"] = pd.to_numeric(df["year"], errors="coerce")
    df = df.loc[df["year"] == int(year)].copy()

    # ----- require ids -----
    if "dg_id" not in df.columns or "player_name" not in df.columns:
        raise ValueError("Fields.xlsx must contain dg_id and player_name to compute YTD")
    ...
    # ----- define "completed" rows by presence of recorded 0/1 outcomes -----
    outcome_cols = [c for c in ["Win", "Top_5", "Top_10", "Top_25", "Made_Cut", "CUT"] if c in df.columns]
    if not outcome_cols:
        # no outcome columns means we can't know what's completed
        return pd.DataFrame(columns=[
            "dg_id", "player_name",
            "ytd_starts", "ytd_wins", "ytd_top10", "ytd_top25", "ytd_made_cuts", "ytd_made_cut_pct"
        ])

    # completed if ANY outcome column is non-null
    completed_mask = df[outcome_cols].notna().any(axis=1)
    df = df.loc[completed_mask].copy()

    if df.empty:
        # nobody has completed events yet for this year
        out = df[["dg_id", "player_name"]].drop_duplicates().copy()
        out["ytd_starts"] = 0
        out["ytd_wins"] = 0
        out["ytd_top10"] = 0
        out["ytd_top25"] = 0
        out["ytd_made_cuts"] = 0
        out["ytd_made_cut_pct"] = np.nan
        return out

    # ----- de-dupe to one row per player/event if event_id exists -----
    if "event_id" in df.columns:
        df["event_id"] = pd.to_numeric(df["event_id"], errors="coerce")
        df = df.dropna(subset=["event_id"]).copy()
        df = df.drop_duplicates(subset=["dg_id", "player_name", "event_id"], keep="last")

    # helper to sum 0/1, treating NaN as 0 AFTER completion filter
    def _sum01(col: str) -> pd.Series:
        if col not in df.columns:
            return pd.Series(0, index=df.index)
        return pd.to_numeric(df[col], errors="coerce").fillna(0).clip(0, 1).astype(int)

    # starts
    if "event_id" in df.columns:
        starts = df.groupby(["dg_id", "player_name"])["event_id"].nunique()
    else:
        starts = df.groupby(["dg_id", "player_name"]).size()

    wins   = df.assign(_v=_sum01("Win")).groupby(["dg_id", "player_name"])["_v"].sum()
    top10  = df.assign(_v=_sum01("Top_10")).groupby(["dg_id", "player_name"])["_v"].sum()
    top25  = df.assign(_v=_sum01("Top_25")).groupby(["dg_id", "player_name"])["_v"].sum()
    made   = df.assign(_v=_sum01("Made_Cut")).groupby(["dg_id", "player_name"])["_v"].sum()

    out = pd.DataFrame({
        "ytd_starts": starts,
        "ytd_wins": wins.reindex(starts.index, fill_value=0),
        "ytd_top10": top10.reindex(starts.index, fill_value=0),
        "ytd_top25": top25.reindex(starts.index, fill_value=0),
        "ytd_made_cuts": made.reindex(starts.index, fill_value=0),
    }).reset_index()

    # pct (undefined if starts==0; but starts should never be 0 after filtering)
        ...
        if not out_parts:
            ...
            if not out_parts:
                ...
                sort_by = st.selectbox(
                    "Sort by",
                    options=sort_choices,
                    index=0,  # default = SG Total if present
